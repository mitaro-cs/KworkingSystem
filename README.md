...
- автоинициализация пустых таблиц в `SQLite`;
- выбор email администратора при запуске;
...
Оба файла создают свое виртуальное окружение, устанавливают зависимости из
`requirements.txt`, открывают браузер и запускают сервер на `http://127.0.0.1:5000`.
При запуске можно ввести email администратора сервиса. Пользователь получит
права администратора после регистрации с этой же почтой.

Порт можно поменять перед запуском:

```bash
PORT=8000 ./start_macos.command
```
...
Для запуска без автоматического открытия браузера задайте `NO_BROWSER=1`.
Для запуска без вопроса про администратора задайте `SKIP_ADMIN_PROMPT=1` или
сразу передайте `COWORKING_ADMIN_EMAIL`.
...
База `coworking.db` инициализируется автоматически пустой: без пользователей,
спотов, бронирований и точек питания. Аватары сохраняются в `static/uploads/avatars/`.
...
| `COWORKING_ADMIN_EMAIL` | email администратора сервиса | спрашивается при запуске |
| `ADMIN_EMAIL` | альтернативное имя переменной для email администратора | пусто |
| `COWORKING_DB_PATH` | путь к SQLite-файлу | `coworking.db` |
| `PUBLIC_URL` | публичный адрес приложения | встроенный `tuna` URL |
| `AUTH_SHOWCASE_IMAGE` | изображение на auth-экране | URL изображения |
| `ASSET_VERSION` | версия статики | `20260427-01` |
...
```bash
COWORKING_SECRET="super-secret" COWORKING_ADMIN_EMAIL="admin@example.edu" PORT=8000 python3 main.py
```
...
- новая база создается пустой, без демо-данных;
- email администратора хранится в настройках базы и синхронизирует права по почте;
